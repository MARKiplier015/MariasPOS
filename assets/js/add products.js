...
function addProduct() {
    ...
    // Check if price and original price are valid numbers
    if (isNaN(parseFloat(price)) || isNaN(parseFloat(ogprice))) {
        alert("Price must be a valid number.");
        return;
    }

    // Assuming products is an array where products are stored
    ...
    // AJAX call to PHP script for adding product
    const xhr = new XMLHttpRequest();
    xhr.open('POST', 'includes/add_product.php', true);
    xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    xhr.onload = function () {
        if (xhr.status == 200) {
            // Handle success, update UI or alert user
    ...
    // Construct the data to send to PHP script
    const params = `name=${encodeURIComponent(name)}&description=${encodeURIComponent(description)}&ogprice=${encodeURIComponent(ogprice)}&price=${encodeURIComponent(price)}`;
    xhr.send(params);
    ...
    // Optionally, you may update the local storage if needed
    localStorage.setItem('products', JSON.stringify(products));
    
    // If you are not using localStorage for products, remove the above line
}
